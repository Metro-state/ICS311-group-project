...
[2] the query took 0.0025 seconds. The rows number prior to the optimization is 10 for SIMPLE type
[3] I indexed the attribute `role` and people_id in the table `movies_people` 
Optimal query:
(SELECT *
FROM movies
  INNER JOIN movie_people ON (movies.movie_id = movie_people.movie_id)
WHERE role = 'Lead Actor' AND people_id = 2);
[4] The query took 0.0018 seconds. The rows number after to the optimization is 1 for PRIMARY type and 10 for UNION type.